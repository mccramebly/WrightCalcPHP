...
<head>
    ...
    <script>
        ...
        function prob() {
            ...
            if (!calc1Yes || x2 === "") {
                document.theForm.output.value += (propor ? "P(p < " + p1 + ") = " : "") + ((x1 != z1) ? "P(x < " + x1 + ") = " : "") + "P(" + przt() + " < " + round4(z1) + ") = " + my(cdf(z1)) + prdf();
            ...
            if (x2 !== "") {
                ...
                if (!calc1Yes) {
                    document.theForm.output.value += (propor ? "P(" + p2 + " < p) = " : "") + ((x2 != z2) ? "P(" + x2 + " < x) = " : "") + "P(" + round4(z2) + " < " + przt() + ") = " + my(1 - cdf(z2)) + prdf();
                    document.theForm.pval.value = my(cdf(z1))
                }
            } else {
                document.theForm.output.value += (propor ? "P(" + x1 / binn + " < p ) = " : "") + ((x1 != z1) ? "P(" + x1 + " < x) = " : "") + "P(" + my(z1) + " < " + przt() + ") = " + my(1 - cdf(z1)) + prdf();
                document.theForm.pval.value = my(cdf(z1))
            }
        }
        // ---------------------------------------------------*/
...
<body onLoad=" if (document.theForm.mean.value == '0') {document.theForm.mean.focus(); document.theForm.mean.select()}">
    ...
    <form name="theForm">
        <div class="calcmenu">
            ...
            <!--<input name="loadbut" Value="Load" type="button" onClick="loadstuff();document.theForm.input.focus()"/>-->
            ...
            <label>Prime</label><input name="disprime" type="checkbox" onClick="if (disprime.checked) dnotprime.checked=false;fixandor();doit()">
            <label>Not Prime</label><input name="dnotprime" type="checkbox" onClick="if (dnotprime.checked) disprime.checked=false;fixandor();doit()">

            <label>From</label> <input type="text" name="xfrom" size=1>
            <label>Thru</label> <input type="text" name="xthru" size=1>
            <label>Condition</label><input type="text" name="xcond" size=20 value="" onClick="document.theForm.xcond.value=xconds[xcondi++]; if (xcondi>=xconds.length) xcondi=0; fixandor(); doit()">
            <input name='xandor' type='button' value='' onClick='switchandor();doit()' /><input name="stats" type="button" value="Stats" onClick="xstats=true;doit();xstats=false">
            <br><br><br>

            <h2>Normal(continuous) Distribution (Gauss/Gauß)</h2>
            <label>Parameters</label><INPUT TYPE="radio" name="CItype" VALUE="Parm" Checked />
            <label>Statistics</label><INPUT TYPE="radio" name="CItype" VALUE="Stat" /><br>
            <label>μ(Mean)</label> <input type="text" name="mean" size=10 value="0" tabindex=1> <br>
            <label>σ(St Dev)</label> <input type="text" name="sdev" size=9 value="1" tabindex=2> <br>
            <label>Sample Size</label> <input type="text" name="sample" size=7 value="1" tabindex=3>
            <label>Data: x1</label> <input type="text" name="x1" size=10 value="0" tabindex=4>
            <label>x2</label><input type="text" name="x2" size=10 value="" tabindex=5>
            <input name="cff" type="button" value="Enter" onClick="zval(); prob(); lastdid=0" tabindex=6><br><br><br>
            <label>Standard score: z1</label> <input type="text" name="z1" size=5 value="1">
            <label>z2</label> <input type="text" name="z2" size=5 value="">
            <input name="xscore" type="button" value="Enter" onClick="xval(); prob(); lastdid=0"><br><br><br>
            <label>Probability</label><input type="text" name="pval" size=10 value="95">
            <label>C.I. or 1-&alpha;</label><input name="zscore" type="button" value="Enter" onClick="CIval(); lastdid=0"><br><br><br>
            <label>Error</label><input type="text" name="error" size=5 value="0.03">

            <label>η =(Z<sub>α/2</sub>σ÷E)² or(Z<sub>α/2</sub>÷E)²pq </label><input name="minsam" type="button" value="Enter" onClick="minsamp(); lastdid=0">
            ...
            <br><br><br><br>
            <h2>Chebyshev</h2>
            <label>K</label><input class="shortinput" type="text" name="Ck" size=12 value="">
            <input name="Ckbut" type="button" value="Enter" onClick="doit(7)">
            <label>P</label><input class="shortinput" type="text" name="Cp" size=12 value="">
            <input name="Cpbut" type="button" value="Enter" onClick="doit(8)">
            <br><br><br>

            <h2>Poisson P(x; μ)</h2>
            <label>X</label><input type="text" name="PX" size=12 value="">
            <label>μ(or λ)</label><input type="text" name="PM" size=12 value="">
