 ...
 <body onLoad="self.focus();document.theForm.input.focus();">
     ...
     <div class="calcmenu">
         ...
         <script>
             ...
             var x8 = 0;
             var y8 = 0;
             var Radius8 = 0 // circum center
             var x9 = 0;
             var y9 = 0;
             var Radius9 = 0 // incenter
             var graphit = false
             var isphone = false
             var graphdata = ''
             var eucenter = ""
             ...
             function center(x) {
                 ...
                 if (x == 0) { // Euler Line
                     ...
                     x61 = (xv[0] + xv[1] + xv[2]) / 3;
                     y61 = (yv[0] + yv[1] + yv[2]) / 3
             ...
             // ----------------------------------*/
             function lcd(a, b) {
                 var a1 = "" + myround(a, true);
                 if (a1.search(/\//) < 0) a1 = 1;
                 else a1 = a1.slice(a1.search(/\//) + 1)
                 var b1 = "" + myround(b, true);
                 if (b1.search(/\//) < 0) b1 = 1;
                 else b1 = b1.slice(b1.search(/\//) + 1)
                 return lcm(a1, b1)
             }
             // ----------------------------------*/
             ...
             // ----------------------------------*/
             function twopoints(x1, y1, x2, y2, prto, x3, y3, n1, n2) {
                 with(Math) {
                     if (n1 == undefined) n1 = '';
                     if (n2 == undefined) n2 = ''
                     if (prto) {
                         document.theForm.output.value += "The distance from "
                     }
                     ...
                     if (prto) {
                         ...
                         vv1 = atan2(y1, x1);
                         vv2 = (450 - vv1 * 180 / PI) % 360;
                         vv3 = myround(sqrt(y1 * y1 + x1 * x1), 4)
             ...
             function calc1() {
                 with(Math) {
                     ...
                     LA = max(angle[0], angle[1], angle[2]);
                     if (!degrees) LA *= PIdeg
                     if (abs((angle[0] - angle[1]) * (angle[0] - angle[2]) * (angle[1] - angle[2])) > 0.0001) {
                         isos = " scalene"
                     } else {
                         isos = (abs(LA - 60) < 0.0001) ? " equilateral" : " isosceles"
                     }
                     ...
                     if (dim3) {
                         ...
                         yv[2] = 0;
                         zv[1] = 0
                         xv[0] = side[2] * cos(angle[1] / PIadj);
                         yv[0] = side[2] * sin(angle[1] / PIadj);
                         zv[2] = 0
                         for (i = 0; i < 3; i++) {
                             xv[i + 3] = xv[i];
                             yv[i + 3] = yv[i]
             ...
             // ----------------------------------*/
             function putpoint(xn, yn, zn) {
                 with(Math) {
                     graphdata += onepoint(xn, yn, zn) // +";"
                     document.theForm.output.value += "("
                     x1 = atan2(yn, xn);
                     x2 = (450 - x1 * 180 / PI) % 360;
                     x3 = my(sqrt(yn * yn + xn * xn))
             ...
             function getpoint() {
                 with(Math) {
                     ...
                     var point = [];
                     var p1 = expr.search(/\(/);
                     if (p1 < 0) return point
                     expr = expr.slice(p1 + 1);
                     p1 = 0;
                     ...
                     do {
                         ...
                         p1++
                     } while (p1 < expr.length)
                     var xyz = expr.slice(0, p1);
                     expr = expr.slice(p1 + 1)
                     point = xyz.split(',')
             ...
             // ----------------------------------*/
             function calc0(exprno, exprd) {
                 with(Math) {
                     angle = [0, 0, 0]
                     side = [0, 0, 0]
                     zv = [0, 0, 0]
                     expr = exprd.replace(/^\s+/, '').replace(/\s+$/, '').replace(/\s\s+/g, 'a');
                     ...
                     if (calcoption < '2') {
                         ...
                         if (calcoption == '1') { // Mid
                             ...
                             for (i = 0; i < 3; i++) {
                                 xv[i] = xmv[i + 1] + xmv[i + 2] - xmv[i];
                                 yv[i] = ymv[i + 1] + ymv[i + 2] - ymv[i]
                             }
                         }
                         ...
                         for (i = 0; i < 5; i++) {
                             side[i] = sqrt(pow((xv[i + 1] - xv[i + 2]), 2) + pow((yv[i + 1] - yv[i + 2]), 2) + pow((zv[i + 1] - zv[i + 2]), 2));
                         }
                         for (i = 0; i < 3; i++) {
                             angle[i] = PIadj * acos((pow(side[i + 1], 2) + pow(side[i + 2], 2) - pow(side[i], 2)) / (2 * side[i + 1] * side[i + 2]))
                         }
                         calc1();
                         return
                     }
                     ...
                     if (expr.search(/[:=]/) > -1) { // expr=expr.replace(/ /g,"") a=3 1/2
                         v = [
                             [, 0, 0],
                             [, 0, 0],
                             [, 0, 0]
                         ...
                         while (expr.search(/[:=]/) > -1) {
                             ...
                             ivvar = expr.substr(iv - 1, 1);
                             ivvarU = ivvar.toUpperCase()
                             expr = expr.slice(iv + 1)
                             iv = (expr + ",").search(/[;,]/)
                         ...
                         if (countsides == 2) {
                             if (sasflag == 0) {
                                 ...
                                 for (i = 0; i < 3; i++) {
                                     if (side[i] * angle[i] != 0) {
                                         firstangle = i;
                                         ratio = sin(angle[i] / PIadj) / side[i]
                                     } else if (side[i] != 0) missingangle = i
                                 }
                                 totangle = 0
                                 for (i = 0; i < 3; i++) {
                                     if (side[i] * ratio > 1) {
                                         document.theForm.output.value += "  Invalid triangle";
                                         return
                                     }
                                 ...
                                 for (i = 0; i < 3; i++)
                                     if (side[i] == 0 && angle[i] != 0) {
                                         missingside = i;
                                         side[i] = sin(angle[i] / PIadj) / ratio
                                     }
                         ...
                         if (countsides > 2) {
                             ...
                             for (i = 0; i < 3; i++)
                                 if (angle[i] == 0) angle[i] = thirdangle
                             for (i = 0; i < 3; i++)
                                 if (side[i] > 0) ratio = sin(angle[i] / PIadj) / side[i]
                     ...
                     while (data.length > 2) {
                         if (calcoption == '2') {
                             ...
                             angle[0] = asin(side[0] * ratio) * PIadj;
                             angle[1] = (degrees ? 180 : PI) - angle[0] - angle[2];
                             ...
                             side[2] = data[1];
                             angle[1] = data[2];
                             angle[2] = (degrees ? 180 : PI) - angle[0] - angle[1];
                             ratio = sin(angle[2] / PIadj) / side[2];
                             side[0] = sin(angle[0] / PIadj) / ratio;
                             side[1] = sin(angle[1] / PIadj) / ratio;
                             ...
                             side[0] = data[2];
                             angle[2] = (degrees ? 180 : PI) - angle[0] - angle[1];
                             ratio = sin(angle[0] / PIadj) / side[0];
                             side[1] = sin(angle[1] / PIadj) / ratio;
                             side[2] = sin(angle[2] / PIadj) / ratio;
                         ...
                         if (calcoption == '3' || calcoption == '8') {
                             angle[0] = (degrees ? 180 : PI) - angle[0];
                             angle[1] = (degrees ? 180 : PI) - angle[0] - angle[2];
                             if (angle[1] > 0) {
                                 side[1] = sin(angle[1] / PIadj) / ratio;
                                 document.theForm.output.value += "\n  *** Second solution:\n";
             ...
             // ----------------------------------*/
             function calc2() {
                 with(Math) { // have all three sides and angles, go for it
                     if (calcoption != '2' && calcoption != '3' && calcoption != '8') {
                         sorted = true;
                         both = [];
                         for (i = 0; i < 3; i++) both.push([angle[i], side[i], vname[i], i])
                     ...
                     xv[0] = trinum + side[1] * cos(angle[2] / PIadj);
                     yv[0] = trinum + side[1] * sin(angle[2] / PIadj);
                     zv[0] = trinum
                     xv[1] = trinum + side[0];
                     yv[1] = trinum;
                     zv[1] = trinum
             ...
             function calc() {
                 ...
                 while (proba.length) {
                     ...
                     if (prob.search(/^ *(\d+|\w)(\)|\. )/) == 0) {
                         probno = prob.match(/^\s*(\d+|\w)(\)|\. )/)[0] + " ";
                         prob = prob.replace(/^ *(\d+|\w)(\)|\. )/, "")
                     }
                     if (prob.search(/.*\([^)]+\,.*\)/) > -1) {
                         if (calcoption != '7' && calcoption != '1') {
                             calcoption = '0'
                         }
                     } else {
                         if (calcoption < '2') calcoption = '2'
                     }
                     if (calcoption < '2') {
                         prob = prob.replace(/\)[^\(\)]*\(/g, ")(");
                         prob = prob.replace(/^[^(]+/, "")
                 ...
                 if (graphit) {
                     xv1 = graphdata.match(/\([^,)]+,[^,)]+/g).sort(function(a, b) {
                         return Number(a.slice(1, a.search(','))) - Number(b.slice(1, b.search(',')))
                     })
                     graphxl = Number(xv1[0].slice(1, xv1[0].search(',')))
                     graphxh = xv1.pop();
                     graphxh = Number(graphxh.slice(1, graphxh.search(',')))
                     graphxm = (graphxl + graphxh) / 2;
                     graphxr = graphxh - graphxl
                     for (ii = 0; ii < xv1.length; ii++) xv1[ii] = xv1[ii].slice(xv1[ii].search(','))
                     xv1 = xv1.sort(function(a, b) {
                         return Number(a.slice(1)) - Number(b.slice(1))
                     })
                     graphyl = Number(xv1[0].slice(1));
                     graphyh = Number(xv1.pop().slice(1))
                     graphym = (graphyl + graphyh) / 2;
                     graphyr = graphyh - graphyl
