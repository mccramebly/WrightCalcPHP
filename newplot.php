...
<body onLoad="self.focus(); document.theForm.input.focus();">
    ...
    <div class="calcmenu">
        ...
        <form name="theForm">
            <canvas id="StoG" width="505" height="505"></canvas>
            ...
            <input name="loadbut" Value="Load" type="button" onClick=" savenow=document.theForm.input.value; loadstuff(); document.theForm.input.value= savenow+document.theForm.input.value; plotfunction(false); document.theForm.input.focus()" /><br><br><br>
            <textarea name="input" rows=11 cols=68 onKeyUp="enter(event)">
y=x^4-10x^2+9[red]
f(x)=-8(x+1)(x-4)[blue]
...
(-2,18)(1,18)(1,-9)(-2,-9)
            ...
            <input name="centerbut" value="center" onclick="centerfunc()" type="button"><br><br><br>
            <textarea rows=11 cols=68 name="debug"></textarea>
        </form>
        <script>
            axis = "black"
            ...
            // -------------------------------------------------------  
            function X2x(X) {
                var x = border + (X - Bx) / dx
                return x
            }
            // -------------------------------------------------------  
            function Y2y(Y) {
                var y = flip - (border + (Y - By) / dy)
                return y
            }
            // -------------------------------------------------------  
            function leftfunc() {
            ...
            function centerfunc() {
                with(Math) {
                    Tx = max(abs(Tx), abs(Fx));
                    Fx = -Tx
                    Ty = max(abs(Ty), abs(Fy));
                    Fy = -Ty
                    plotfunction(true)
                }
            }
            ...
            function doaxis() {
                ...
                if (Fy * Ty < 0) {
                    ...
                    ctx.moveTo(2, flip - ((0 - By) / dy + border));
                    ctx.lineTo(1, flip - ((0 - By) / dy + border));
                    ctx.lineTo(mxi + border + 1, flip - ((0 - By) / dy + border));
                    ctx.stroke()
                ...
                if (Fx * Tx < 0) {
                    ...
                    ctx.lineWidth = border;
                    ctx.strokeStyle = axis;
                    ctx.moveTo((0 - Bx) / dx + border, 2);
                    ctx.lineTo((0 - Bx) / dx + border, 1);
                    ctx.lineTo((0 - Bx) / dx + border, myi + border + 1);
                    ctx.stroke()
                    // y marks
            ...
            // --------------------------------------------
            function plotfunction(move) {
                with(Math) {
                    hadgraph = false;
                    needY = true;
                    curvei = 0
                    ctx.fillStyle = "#FFFFFF";
                    ctx.fillRect(0, 0, mxi + 10, myi + 10)
                    if (move) {
                        dx = (Tx - Fx) / (mxi - 1);
                        dy = (Ty - Fy) / (myi - 1);
                        By = Fy - border * dy;
                        ...
                        ctx.fillRect(X2x(OFx), Y2y(OTy), X2x(OTx) - X2x(OFx), Y2y(OFy) - Y2y(OTy));
                        needY = false;
                        doaxis()
                    } else {
                        OFx = Fx;
                    ...
                    while (expr.length > 0) {
                        ...
                        if (expr0.search(/\[/) > -1) {
                            ...
                            curvecolor = expr0.slice(expr01 + 1, expr02);
                            expr0 = expr0.slice(0, expr01)
                        } else {
                            curvecolor = curvecolors[curvei];
                            curvei++
                        }
                        ...
                        if (expr0.search(/,/) == -1) { // calc points 
                            if (fofx.search(/x/i) == -1) {
                                plotpoint(eval(fofx), 0);
                                continue
                            }
                            if (needY) {
                                Fy = f(Fx);
                        ...
                        while (expr0.search(/,/) > -1) {
                            ...
                            Px = eval(expr0.slice(0, expr1e));
                            Py = eval(expr0.slice(expr1e + 1, expr2e))
